...
static int tt_device_open_flags(const char* chardev_path, tt_device_t** out_dev, int open_flags) {
    ...
    dev->fd = open(chardev_path, open_flags);
    if (dev->fd == -1) {
        int e = errno;
        free(dev);
        return -e;
...
int tt_device_open(const char* chardev_path, tt_device_t** out_dev) {
    return tt_device_open_flags(chardev_path, out_dev, O_RDWR | O_CLOEXEC);
}

int tt_device_open_power_aware(const char* chardev_path, tt_device_t** out_dev) {
    return tt_device_open_flags(chardev_path, out_dev, O_RDWR | O_CLOEXEC | O_APPEND);
}
